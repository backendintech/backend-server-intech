...
const createProduct = async (req, res, next) => {
  try {
    const { name, description, keypoints, sell_price, category } = req.body;
...
const updateProduct = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { name, description, keypoints, sell_price, category } =
      req.body;
    ...
    if (!req.files) {
      ...
      result.keypoints = JSON.parse(keypoints);
